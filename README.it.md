...
| 🚀 [`deploy-and-test.md`](deploy-and-test.md) | Il loop ad alta frequenza pull → edit → deploy → trigger → verify; token `az`, PATCH di `clientdata`, il **tranello della ri-registrazione del trigger** (lo spreca-tempo n.1), come lanciare i trigger **Button / PowerApps V2** con lo swap temporaneo a `kind:Http`, lettura degli output via `outputsLink` e di ciò che è stato *realmente* inviato via `inputsLink`, semantica dei trigger Dataverse (nessuna pre-image, trucco del backfill con `filteringattributes`) e il tranello **DateOnly UserLocal** (data indietro di un giorno). |
| 🆕 [`creating-flows.md`](creating-flows.md) | Creare un flow **da zero** via API: `schemaVersion` a root del `clientdata`, `POST workflows` → activate → `AddSolutionComponent`, perché **chi chiama deve possedere le connessioni**, riuso di connection reference già bindate senza solution import, differenze "normali" fra live e build, e la forma dello script (`pull/build/deploy/verify/test/rollback`). |
| 📝 [`document-generation.md`](document-generation.md) | Generazione documenti: matrice connettori (Word/Excel/OneDrive/PDF), content control Word per `w:id` (**anche dentro le caselle di testo ancorate**), come rendere popolabile il file solo-grafica del designer, il tranello **72 vs 300 dpi** (PDF sgranato), formato pagina omogeneo per i PDF mergiati, perché il docx del connettore è **illeggibile in Word** (consegnare il PDF), workaround per liste a lunghezza variabile, tranelli Excel→PDF e allegati email. |
| 🔐 [`auth-environments.md`](auth-environments.md) | Auth e identità: drift `pac`/`az`, quale identità accede a quale drive (il tranello OneDrive), chi può creare un flow, connection reference & binding, mappa errore → causa. |
